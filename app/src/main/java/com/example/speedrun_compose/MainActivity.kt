        ...
        setContent {
            SpeedRunAppTheme {
                Surface(modifier = Modifier.padding(), color = MaterialTheme.colorScheme.background) {
                    RequestOverlayPermission() // Composable para gestionar permisos
                }
            }
        }
    }

    // Registro para manejar el resultado de la solicitud de permisos
    @RequiresApi(Build.VERSION_CODES.M)
    private val requestOverlayPermission =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (Settings.canDrawOverlays(this)) {
                // Si el permiso es concedido, iniciar el servicio flotante
                startFloatingMenuService()
            } else {
                // Si el permiso no fue concedido, mostrar un mensaje
                Toast.makeText(this, "No se otorgaron los permisos necesarios", Toast.LENGTH_SHORT).show()
            }
        }

    // Método para iniciar el servicio flotante
    @RequiresApi(Build.VERSION_CODES.M)
    private fun startFloatingMenuService() {
        if (Settings.canDrawOverlays(this)) {
            val intent = Intent(this, FloatingMenuService::class.java)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                startForegroundService(intent) // Usar startForegroundService para Android O y superior
            } else {
                startService(intent) // Para versiones anteriores de Android
            }
            Log.d("MainActivity", "Servicio flotante iniciado")
        } else {
            Log.d("MainActivity", "Permiso no concedido para dibujar sobre otras apps")
        }
    }

    // Composable que muestra un botón para pedir el permiso
    @RequiresApi(Build.VERSION_CODES.M)
    @Composable
    fun RequestOverlayPermission() {
        if (!Settings.canDrawOverlays(this)) {
            // Si no tiene el permiso, mostrar el botón para solicitarlo
            Button(onClick = {
                // Lanza la actividad para pedir el permiso
                val intent = Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION, Uri.parse("package:$packageName"))
                requestOverlayPermission.launch(intent)
            }) {
                Text("Permitir sobreponer contenido")
            }
        } else {
            // Si ya tiene el permiso, iniciar el servicio
            startFloatingMenuService()
        }
    }

    // En onResume verificamos si el permiso fue otorgado y si no, iniciamos el servicio
    @RequiresApi(Build.VERSION_CODES.M)
    override fun onResume() {
        super.onResume()
        // Verificamos el permiso para dibujar sobre otras aplicaciones
        if (Settings.canDrawOverlays(this)) {
            // Si el permiso es concedido, iniciar el servicio flotante
            startFloatingMenuService()
        } else {
            // Si no tiene el permiso, mostrar el botón para solicitarlo
            Log.d("MainActivity", "Permiso de superposición no concedido aún")
        }
    }
}
...
fun SpeedRunAppNavigation() {
    ...
    var games by remember { mutableStateOf(listOf("Super Mario Bros", "The Legend of Zelda", "Minecraft", "Dark Souls")) }
    ...
    LaunchedEffect(navController) {
        snapshotFlow { navController.currentBackStackEntry?.destination?.route }
            .collect { route ->
                ...
                currentRoute = route
                Log.d("NavRoute", "Ruta actual: $route") // Log para ver todas las rutas, incluyendo "game_details"
            }
    }
    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = "game_list", modifier = Modifier.padding(paddingValues)) {
            ...
            composable("game_details/{gameName}") { backStackEntry ->
                ...
                Log.d("NavRoute", "Ruta actual en Game Details: ${navController.currentBackStackEntry?.destination?.route}")
            }
    ...
    LaunchedEffect(showSelectDialog) {
        if (showSelectDialog && selectedGame.isBlank() && games.isNotEmpty()) {
            selectedGame = games.first() // Si no hay juego seleccionado, se selecciona el primero de la lista
        }
    }
                ...
                Button(
                    onClick = {
                        if (newGameName.text.isNotBlank()) {
                            ...
                            selectedGame = newGameName.text // Asegurarse de que el juego seleccionado también tenga el nuevo nombre
                            newGameName = TextFieldValue("") // Limpiar el campo de texto
                        }
...
@Composable
fun GameListScreen(games: List<String>, onAddGame: () -> Unit, onGameSelected: (String) -> Unit) {
    // Lista de juegos
    LazyColumn {
        items(games) { game ->
                        ...
                        items(sections) { section ->
                            Text(
                                ...
                                modifier = Modifier
                                    .padding(8.dp)
                                    .clickable {
                                        selectedSection = section // Permitir seleccionar sección
                                    }
                            )
                        }
                    }
